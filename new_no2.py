...
mode = "all"  # デフォルトモード
if len(sys.argv) > 1:
    if sys.argv[1] == "--defined-only":
        mode = "defined-only"
    elif sys.argv[1] == "--undefined-only":
        mode = "undefined-only"
...
date_pattern2 = re.compile(r'\d{4}[/-]\d{2}[/-]\d{2}\s\d{2}:\d{2}:\d{2}(?:\.\d+)?')
...
def preprocess_log(log_line):
    ...
    # IPアドレスの置換（IP_ADDRではなくIPADDRに）
    processed = ip_pattern.sub("IPADDR", processed)
...
def classify_log(log_line, processed_line, categories):
    ...
    if len(similarities) > 0 and np.max(similarities) > 0.8:  # より厳しい類似度の閾値
...
def main():
    ...
    with open('analy.log', 'r', encoding='utf-8') as log_file:
        for line_num, log_line in enumerate(log_file):
            ...
            if mode == "all":
                print(result)
            elif mode == "defined-only" and category != "undefined":
                print(result)
            elif mode == "undefined-only" and category == "undefined":
                print(result)
            
            if category == "undefined":
                undefined_logs.append((line_num, log_line, processed_line))
    
    # undefined-onlyモードまたはdefined-onlyモードなら対話処理をスキップ
    if mode != "all":
        return
...
# ex)
# ./02_categoman_no1.py
# ./02_categoman_no1.py --defined-only
# ./02_categoman_no1.py --undefined-only
#
